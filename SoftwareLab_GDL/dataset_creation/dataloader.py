...
print("当前使用的 GPU:", torch.cuda.get_device_name(0) if torch.cuda.is_available() else "无 GPU")
#%%
relation_type = {
    "no_relation": 0,
    "superpose_on": 1,
...
def generate_relation_label(label_file_path, relation_file_path):
    ...
    relation_edge = []
    relation_edge_type = []
    # relation是一组seg的集合，对每组seg里的face建立关系边
...
def generate_dataset(
    ...
    attr_stat_path = "../data/attr_stat.json"
...
# 加载 .pt 文件（模型或数据）
data = torch.load("../data/dataset/test.pt")
