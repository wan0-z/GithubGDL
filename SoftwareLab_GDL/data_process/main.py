...
def process_one_file(args):
    file_path, config, output_path = args
    try:
...
if __name__ == '__main__':
    ...
    step_files = list(step_path.glob("20250701*_result.step"))
    print(step_files)

    pool = Pool(processes=num_workers, initializer=initializer)
    try:
        results = list(tqdm.tqdm(
    pool.imap(process_one_file, zip(step_files, repeat(attribute_config), repeat(output_path))),
